...
// Definire un gruppo di route con il prefisso comune
Route::prefix('admin/qlik_items')->group(function () use ($controllers_base_path) {
    $controller = $controllers_base_path .  'AdminQlikItemsController';
    
    // Percorsi per la gestione dei contenuti e degli accessi
    Route::get('content/{qlik_item_id}',$controller . '@content_view');
    Route::get('access/{qlik_item_id}',$controller . '@access');
    Route::get('access/{qlik_item_id}/alert/{alert_id}',$controller . '@access');
    Route::post('{qlik_item_id}/auth',$controller . '@add_authorization');
    Route::get('{qlik_item_id}/deauth/{group_id}',$controller . '@remove_authorization');
    
    // Percorsi per la gestione dei tenant
    Route::get('tenant/{qlik_item_id}',$controller . '@tenant');
    Route::get('tenant/{qlik_item_id}/alert/{alert_id}',$controller . '@tenant');
    Route::post('{qlik_item_id}/add_tenant',$controller . '@add_tenant');
    Route::get('{qlik_item_id}/remove_tenant/{tenant_id}',$controller . '@remove_tenant');
});

// Percorso separato per il controller QlikItemsController
...
// Gruppo di rotte per il controller AdminChatAIController
Route::prefix('admin/chat_ai')->group(function () use ($controllers_base_path) {
    $controller = $controllers_base_path . 'AdminChatAIController';

    //if(crocodicstudio\crudbooster\helpers\LicenseHelper::isActiveChatAI()) {

        // Route di contenuti
        Route::get('content/{chat_ai_id}', $controller . '@content_view');
        
        // Route di accesso
        Route::get('access/{chat_ai_id}/alert/{alert_id}', $controller . '@access');
        Route::get('access/{chat_ai_id}', $controller . '@access');
        
        // Autorizzazione
        Route::post('{chat_ai_id}/auth', $controller . '@add_authorization');
        Route::get('{chat_ai_id}/deauth/{group_id}', $controller . '@remove_authorization');
        
        // Tenant
        Route::get('tenant/{chat_ai_id}/alert/{alert_id}', $controller . '@tenant');
        Route::get('tenant/{chat_ai_id}', $controller . '@tenant');
        Route::post('{chat_ai_id}/add_tenant', $controller . '@add_tenant');
        Route::get('{chat_ai_id}/remove_tenant/{tenant_id}', $controller . '@remove_tenant');
        
        // Invio messaggi
        Route::post('send_message', $controller . '@send_message');
        Route::post('send_message_agent', $controller . '@send_message_agent');
    //}
});
