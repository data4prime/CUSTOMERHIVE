...
function update_objects(select){
    ...
    iframe.src = '/mashup-objects/' + mashup_id + '/{{$componentID}}'+'/{{isset($config->object) ? $config->object : ""}}';
...
<iframe src='/mashup-objects/{{ isset($mashup->id) ? $mashup->id : ""}}/{{$componentID}}/{{isset($config->object) ? $config->object : ""}}'  id="configuration" frameborder="0" style="width: 100%;height:30px;"></iframe>
...
<input type="hidden" id="mashup_app_hidden" value="{{$config->mashups}}">
    </div>
