<div class='mb-3 row form-datepicker {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}'
    ...
    <div class=" {{ isset($col_width)? $col_width :'col-sm-10'}}">
        ...
        */
        ...
        $datamodal_value = isset($datamodal_value->$datamodal_field) ? $datamodal_value->$datamodal_field : '';
        ...
        ?>
            ...
            <div id='{{$name}}' class="input-group">
                <input type="hidden" name="{{$name}}" class="input-id" value="{{$value}}" {{$required?"required":""}}>
