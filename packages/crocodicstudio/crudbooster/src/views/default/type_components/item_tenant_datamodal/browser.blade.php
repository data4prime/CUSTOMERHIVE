...
<?php
  ...
  $result = DB::table('tenants')
                ...
                $query->select(DB::raw(1))
                      ...
                      ->whereRaw('tenants_allowed.item_id = '.Request::get('select_to').' AND tenants_allowed.tenant_id = tenants.id');
            });
  if($q){
  ...
  <tbody>
    ...
    <tr>
      ...
      <?php
            ...
            // quel formato produceva una chiave vuota nel JSON, che in JS
            // ($('#' + chiave)) diventava il selettore invalido '#', mandando
            // in eccezione lo script prima che il popup potesse chiudersi.
            $select_data_result = [];
