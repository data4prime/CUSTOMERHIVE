...
$val=$lab='' ;
if(strpos($enum, ' |' ) !==FALSE) { $draw=explode("|",$enum); $val=$draw[0]; $lab=$draw[1]; } else { $val=$lab=$enum; }
            $select=($value==$val) ? "selected" : "" ; echo "<option " . $select ."value='". $val ."'>".$lab."</option>
            "; }
            @endphp @endif @if(isset($form[' datatable']))
            @if(isset($form['relationship_table'])) $select_table=explode(',', $form['datatable'])[0];
            $select_title=explode(',', $form['datatable'])[1]; $select_where=isset($form['datatable_where']) ?
            $form['datatable_where'] : '' ; $pk=CRUDBooster::findPrimaryKey($select_table);
            $result=DB::table($select_table)->select($pk,
            $select_title);
            if ($select_where) {
            ...
            $params = explode(" |", $form['datatable_orig']); if(!isset($params[2])) $params[2]="id" ;
