                    ...
                    <div class='col-sm-10'>
                        <div class="card card-default">
                            ...
                            <div class="card-body child-form-area">
                                ...
                                <div class='mb-3 row'>
                                    ...
                                    <div class="col-sm-10">
                                        ...
                                        <input id='{{$name_column}}' type='text' {{
                                            isset($col['max'])?"maxlength='".$col[' max']."'":"" }}
                                            name='child-{{$col["name"]}}' class='form-control {{ isset($col['
                                            required'])?"required":""}}' {{(isset($col['readonly']) &&
                                        ...
                                        <label class="radio-inline">
                                            <input type="radio" name="child-{{$col['name']}}" class='{{ ($e==0 && isset($col["required"]))?"required":""}}
                                                {{$name_column}}' value="{{$radio_value}}" {{ ($e==0 &&
                                                $col['required'])?" checked":""}}> {{$radio_label}}
                                        </label>
                                        @php endforeach;@phpend
                                        @php endif;@phpend
                                        ...
                                        <div id='{{$name_column}}' class="input-group">
                                            ...
                                            <input type="text"
                                                class="form-control input-label {{ isset($col['required'])?"
                                                required":""}}" readonly>
                                        ...
                                        @push('bottom')
                                        <script type="text/javascript">
                                            var url_{{ $name_column }} = "{{CRUDBooster::mainpath('modal-data')}}?table={{$col['datamodal_table']}}&columns=id,{{$col['datamodal_columns']}}&name_column={{$name_column}}&where={{urlencode($col['datamodal_where'])}}&select_to={{ urlencode($col['datamodal_select_to']) }}&columns_name_alias={{ isset($col['datamodal_columns_alias']) && urlencode($col['datamodal_columns_alias'])}}";
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <iframe id='iframe-modal-{{$name_column}}'
                                                            style="border:0;height: 430px;width: 100%" src=""></iframe>
                                                    </div>
                                        ...
                                        <input id='{{$name_column}}' type='number' {{ isset($col['min'])?"min='".$col['
                                            min']."'":"" }} {{ isset($col['max'])?"max='$col[max]'":"" }} name="
                                            child-{{$col['name']}}" class='form-control {{isset($col['
                                            required'])?"required":""}}' {{( isset($col['readonly']) &&
                                            $col['readonly']===true)?"readonly":""}} />
                                        @elseif($col['type']=='textarea')
                                        <textarea id='{{$name_column}}' name='child-{{$col["name"]}}'
                                            class='form-control {{isset($col["required"])?"required":""}}'
                                        ...
                                        <script type="text/javascript">
                                            ...
                                            function prepareUpload{ { $name_column } } (event) {
                                                var max_size = {{ isset($col['max']) ? $col['max'] : 2000
                                            }};
                                        ...
                                        <select id='{{$name_column}}' name='child-{{$col["name"]}}'
                                            class='form-control select2 {{isset($col["required"])?"required":""}}' {{(
                                            isset($col['readonly']) && $col['readonly']===true)?"readonly":""}}>
                                            ...
                                            $data = CRUDBooster::get($tableJoin, $col['datatable_where'], "$titleField
                                            ASC");
                                            }
                                            foreach ($data as $d) {
                                ...
                                $formula = str_replace("[".$c['name']."]", "\$('#".$name.$c['name']."').val()",
                                $formula);
                                }
                                @phpend
                                @push('bottom')
                    ...
                    <div class="card-body no-padding table-responsive" style="max-height: 400px;overflow: auto;">
                        <table id='table-{{$name}}' class='table table-striped table-bordered'>
                            ...
                            <tbody>
                                ...
                                $data_child->addselect($c['datamodal_table'].'.'.$datamodal_title.' as
                                '.$datamodal_table.'_'.$datamodal_title);
                                ...
                                <tr>
                                    ...
                                    <td class="{{$col['name']}}">
                                        ...
                                        echo "<span class='td-label'>";
                                            ...
                                            echo "</span>";
                                        echo "<input type='hidden' name='".$name."-".$col[' name']."[]'
                                            value='".$d->{$col[' name']}."' />";
                                        }
                                        if ($col['dataenum']) {
                                        echo "<span class='td-label'>";
                                        ...
                                        echo "<a href='".asset($d->{$col[' name']})."' data-lightbox='roadtrip'><img
                                                data-label='$filename' src='".asset($d->{$col[' name']})."' width='50px'
                                                height='50px' /></a>";
                                        echo "<input type='hidden' name='".$name."-".$col[' name']."[]'
                                            value='".$d->{$col[' name']}."' />";
                                        } else {
                                        echo "<a data-label='$filename' href='".asset($d->{$col['
                                            name']})."'>$filename</a>";
                                        echo "<input type='hidden' name='".$name."-".$col[' name']."[]'
                                            value='".$d->{$col[' name']}."' />";
                                        }
